...
export function Radar({ data }: { data: RadarData }) {
  ...
  return <main className="min-h-screen bg-[#070a10] text-slate-100"><div className="radar-glow pointer-events-none fixed inset-0" /><div className="relative mx-auto max-w-6xl px-4 pb-20 pt-8 sm:px-6 sm:pt-12">
    ...
    <Tabs defaultValue="singer" className="gap-6"><TabsList className="grid h-auto w-full grid-cols-1 gap-2 overflow-visible rounded-xl border border-white/10 bg-white/[0.04] p-1.5 sm:w-fit sm:grid-cols-2"><TabsTrigger value="singer" className="min-h-11 w-full flex-none justify-between gap-2 rounded-lg px-4 text-sm data-[state=active]:bg-cyan-300 data-[state=active]:text-slate-950">Zpěv hledá <span className="rounded-full bg-black/10 px-2 py-0.5 text-xs">{data.singerSeeking.length}</span>{newSinger > 0 && <span className="text-xs font-bold">+{newSinger} nové</span>}</TabsTrigger><TabsTrigger value="interesting" className="min-h-11 w-full flex-none justify-between gap-2 rounded-lg px-4 text-sm data-[state=active]:bg-cyan-300 data-[state=active]:text-slate-950">Obecně zajímavé <span className="rounded-full bg-black/10 px-2 py-0.5 text-xs">{data.interesting.length}</span>{newInteresting > 0 && <span className="text-xs font-bold">+{newInteresting} nové</span>}</TabsTrigger></TabsList>
