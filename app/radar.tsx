...
import { ArrowUpRight, Bookmark, CalendarDays, Check, CheckCheck, Link2, MapPin, Sparkles } from "lucide-react";
...
function AdCard({ ad, isRead, isSaved, onRead, onSave }: { ad: Ad; isRead: boolean; isSaved: boolean; onRead: () => void; onSave: () => void }) {
  return <article className={`group relative overflow-hidden rounded-2xl border p-5 transition hover:-translate-y-0.5 ${isRead ? "border-white/8 bg-white/[0.02] opacity-75 hover:opacity-100" : "border-cyan-300/45 bg-cyan-300/[0.075] shadow-[inset_3px_0_0_rgba(103,232,249,0.65)] hover:border-cyan-300/65"}`}>
    <div className="mb-4 flex flex-wrap items-center gap-2 text-sm text-slate-400">
      ...
      <span className="rounded-full border border-white/10 bg-white/[0.045] px-2.5 py-1 text-xs font-semibold text-slate-300">{ad.source || "Hudební bazar"}</span>
...
function SavedFilter({ active, count, onToggle }: { active: boolean; count: number; onToggle: () => void }) {
  return <Button type="button" size="sm" variant="outline" aria-pressed={active} onClick={onToggle} className={`min-h-9 rounded-full shadow-none ${active ? "border-amber-300/40 bg-amber-300 text-slate-950 hover:bg-amber-300 hover:text-slate-950" : "border-white/10 bg-white/[0.035] text-slate-300 hover:bg-white/10 hover:text-white"}`}><Bookmark className="size-4" />Uložené <span className="text-xs">{count}</span></Button>;
}
...
export function Radar({ data }: { data: RadarData }) {
  ...
  const markAllRead = () => { const next = new Set(readKeys); [...currentAds, ...savedAds.map((item) => item.ad)].forEach((ad) => next.add(adKey(ad))); setReadKeys(next); window.localStorage.setItem(READ_KEY, JSON.stringify([...next])); };
  ...
  const singerBase = status.singer === "saved" ? byStatus(singerAll, "saved") : unreadOnly ? (ready ? data.singerSeeking.filter((ad) => !isRead(ad)) : []) : byStatus(data.singerSeeking, status.singer);
  const interestingStatusBase = status.interesting === "saved" ? byStatus(interestingAll, "saved") : unreadOnly ? (ready ? data.interesting.filter((ad) => !isRead(ad)) : []) : byStatus(data.interesting, status.interesting);
  ...
  return <main className="min-h-screen bg-[#070a10] text-slate-100"><div className="radar-glow pointer-events-none fixed inset-0" /><div className="relative mx-auto max-w-6xl px-4 pb-20 pt-8 sm:px-6 sm:pt-12">
    <header className="mb-9 flex flex-col gap-6 border-b border-white/10 pb-8 md:flex-row md:items-end md:justify-between"><div><h1 className="text-4xl font-bold tracking-[-0.04em] text-white sm:text-6xl">Hudební radar<span className="text-cyan-300">.</span></h1><p className="mt-3 max-w-2xl text-base leading-7 text-slate-400 sm:text-lg">Zajímaví lidé a vznikající projekty z českých hudebních inzercí — pro hledání zpěvu, networking a objevování nové hudby.</p>{ready && newSinger + newInteresting > 0 && <div className="mt-4 inline-flex items-center gap-2 rounded-full border border-cyan-300/30 bg-cyan-300/10 px-3 py-1.5 text-sm font-bold text-cyan-200"><Sparkles className="size-4" />{newSinger + newInteresting} nových inzerátů</div>}</div><div className="shrink-0 text-sm leading-6 text-slate-500"><span className="block font-semibold text-slate-300">Poslední kontrola</span>{checkedAt.format(new Date(data.updatedAt))} · okno {data.windowDays} dní</div></header>
    <div className="mb-5 flex flex-col gap-3 rounded-2xl border border-cyan-300/20 bg-cyan-300/[0.06] px-4 py-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center justify-between gap-4"><label htmlFor="unread-only" className="cursor-pointer"><span className="block font-semibold text-white">Pouze nové inzeráty</span><span className="mt-0.5 block text-sm text-slate-400">Skrýt všechno, co už bylo označeno jako přečtené</span></label><Switch id="unread-only" checked={unreadOnly} onCheckedChange={setUnreadOnly} aria-label="Zobrazovat pouze nové inzeráty" className="shrink-0 data-[state=checked]:bg-cyan-300 sm:hidden" /></div><div className="flex items-center gap-3"><Button type="button" size="sm" variant="outline" onClick={markAllRead} disabled={!ready || newSinger + newInteresting === 0} className="min-h-10 flex-1 border-white/10 bg-white/[0.045] text-slate-200 hover:bg-white/10 hover:text-white disabled:opacity-45 sm:flex-none"><CheckCheck className="size-4" />Označit vše jako přečtené</Button><Switch id="unread-only-desktop" checked={unreadOnly} onCheckedChange={setUnreadOnly} aria-label="Zobrazovat pouze nové inzeráty" className="hidden shrink-0 data-[state=checked]:bg-cyan-300 sm:inline-flex" /></div></div>
    <Tabs defaultValue="singer" className="gap-6"><TabsList className="!flex !h-auto w-full flex-col items-stretch gap-2 overflow-visible rounded-xl border border-white/10 bg-white/[0.04] p-1.5 sm:w-fit sm:flex-row sm:items-center"><TabsTrigger value="singer" className="!h-auto min-h-11 w-full flex-none justify-between gap-2 whitespace-normal rounded-lg px-4 py-2 text-left text-sm leading-tight sm:w-auto data-[state=active]:bg-cyan-300 data-[state=active]:text-slate-950">Zpěv hledá <span className="ml-auto rounded-full bg-black/10 px-2 py-0.5 text-xs">{unreadOnly ? newSinger : data.singerSeeking.length}</span>{!unreadOnly && newSinger > 0 && <span className="text-xs font-bold">+{newSinger} nové</span>}</TabsTrigger><TabsTrigger value="interesting" className="!h-auto min-h-11 w-full flex-none justify-between gap-2 whitespace-normal rounded-lg px-4 py-2 text-left text-sm leading-tight sm:w-auto data-[state=active]:bg-cyan-300 data-[state=active]:text-slate-950">Obecně zajímavé <span className="ml-auto rounded-full bg-black/10 px-2 py-0.5 text-xs">{unreadOnly ? newInteresting : data.interesting.length}</span>{!unreadOnly && newInteresting > 0 && <span className="text-xs font-bold">+{newInteresting} nové</span>}</TabsTrigger></TabsList>
      <TabsContent value="singer" className="space-y-4"><div className="mb-5"><h2 className="text-2xl font-semibold tracking-tight text-white">Zpěv hledá kapelu nebo projekt</h2><p className="mt-1 text-slate-400">Pouze Praha a Středočeský kraj; bez coverů, tribute a zábavových kapel.</p></div><div className="mb-6 rounded-2xl border border-white/10 bg-white/[0.025] p-4">{unreadOnly ? <SavedFilter active={status.singer === "saved"} count={counts(singerAll).saved} onToggle={() => setStatus((s) => ({ ...s, singer: s.singer === "saved" ? "all" : "saved" }))} /> : <StatusFilters value={status.singer} onChange={(value) => setStatus((s) => ({ ...s, singer: value }))} counts={counts(singerAll)} />}</div>{cards(singerShown, "singer")}</TabsContent>
      <TabsContent value="interesting" className="space-y-4"><div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-2xl font-semibold tracking-tight text-white">Obecně zajímavé inzeráty</h2><p className="mt-1 text-slate-400">Moderní rock a metal, vlastní tvorba a nové projekty vhodné ke sledování, propojení nebo podpoře.</p></div><div className="shrink-0"><label className="mb-1.5 block text-sm font-semibold text-slate-400">Řazení</label><Select value={sort} onValueChange={setSort}><SelectTrigger className="min-w-44 border-white/10 bg-white/[0.035] text-slate-200"><SelectValue /></SelectTrigger><SelectContent className="border-white/10 bg-[#111821] text-slate-100"><SelectItem value="newest">Nejnovější</SelectItem><SelectItem value="score">Nejvyšší shoda</SelectItem></SelectContent></Select></div></div><div className="mb-6 space-y-3 rounded-2xl border border-white/10 bg-white/[0.025] p-4">{unreadOnly ? <SavedFilter active={status.interesting === "saved"} count={counts(interestingAll).saved} onToggle={() => setStatus((s) => ({ ...s, interesting: s.interesting === "saved" ? "all" : "saved" }))} /> : <StatusFilters value={status.interesting} onChange={(value) => setStatus((s) => ({ ...s, interesting: value }))} counts={counts(interestingAll)} />}<TopicFilters ads={interestingStatusBase} active={filters} onToggle={toggleFilter} onClear={() => setFilters(new Set())} /></div>{cards(interestingShown, "interesting")}</TabsContent>
    </Tabs></div></main>;
}
