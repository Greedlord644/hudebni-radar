...
const checkedAt = new Intl.DateTimeFormat("cs-CZ", { day: "numeric", month: "long", year: "numeric", hour: "2-digit", minute: "2-digit" });
...
function AdCard({ ad, isRead, isSaved, onRead, onSave }: { ad: Ad; isRead: boolean; isSaved: boolean; onRead: () => void; onSave: () => void }) {
  return <article className={`group relative overflow-hidden rounded-2xl border p-5 transition hover:-translate-y-0.5 ${isRead ? "border-white/8 bg-white/[0.02] opacity-75 hover:opacity-100" : "border-cyan-300/45 bg-cyan-300/[0.075] shadow-[inset_3px_0_0_rgba(103,232,249,0.65)] hover:border-cyan-300/65"}`}>
    ...
    <div className="mt-5 flex flex-wrap items-center gap-2 border-t border-white/8 pt-4 text-sm">
      <span className="mr-auto text-slate-500">{ad.author}</span>
      <Button type="button" size="sm" variant="outline" aria-pressed={isRead} onClick={onRead} className="border-white/10 bg-white/[0.035] text-slate-300 hover:bg-white/10 hover:text-white"><Check className="size-4" />{isRead ? "Přečteno" : "Označit jako přečtené"}</Button>
      <Button type="button" size="sm" variant="outline" aria-pressed={isSaved} onClick={onSave} className={isSaved ? "border-amber-300/30 bg-amber-300/10 text-amber-200 hover:bg-amber-300/15" : "border-white/10 bg-white/[0.035] text-slate-300 hover:bg-white/10 hover:text-white"}><Bookmark className="size-4" />{isSaved ? "Uloženo" : "Uložit"}</Button>
      <a className="inline-flex min-h-8 items-center gap-1.5 px-2 font-semibold text-cyan-300 outline-none hover:text-cyan-200 focus-visible:ring-2 focus-visible:ring-cyan-300" href={ad.url} target="_blank" rel="noreferrer">Otevřít inzerát <ArrowUpRight className="size-4" /></a>
    </div>
  </article>;
}
...
function TopicFilters({ ads, active, onToggle, onClear }: { ads: Ad[]; active: Set<FilterKey>; onToggle: (value: FilterKey) => void; onClear: () => void }) {
  ...
  return <div className="flex flex-wrap items-center gap-2"><span className="mr-1 text-sm font-semibold text-slate-400">Obsah</span><Button type="button" size="sm" variant="outline" aria-pressed={!active.size} onClick={onClear} className={`rounded-full border-white/10 shadow-none ${!active.size ? "bg-cyan-300 text-slate-950" : "bg-white/[0.035] text-slate-300"}`}>Vše <span className="text-xs">{ads.length}</span></Button>{options.map(({ key, label }) => <Button key={key} type="button" size="sm" variant="outline" aria-pressed={active.has(key)} onClick={() => onToggle(key)} className={`rounded-full border-white/10 shadow-none ${active.has(key) ? "bg-cyan-300 text-slate-950" : "bg-white/[0.035] text-slate-300"}`}>{label} <span className="text-xs">{count(key)}</span></Button>)}</div>;
}
...
function StatusFilters({ value, onChange, counts }: { value: StatusFilter; onChange: (value: StatusFilter) => void; counts: Record<StatusFilter, number> }) {
  ...
  return <div className="flex flex-wrap items-center gap-2"><span className="mr-1 text-sm font-semibold text-slate-400">Stav</span>{options.map(({ key, label }) => <Button key={key} type="button" size="sm" variant="outline" aria-pressed={value === key} onClick={() => onChange(key)} className={`rounded-full border-white/10 shadow-none ${value === key ? key === "new" ? "bg-cyan-300 text-slate-950" : key === "saved" ? "bg-amber-300 text-slate-950" : "bg-white text-slate-950" : "bg-white/[0.035] text-slate-300"}`}>{label} <span className="text-xs">{counts[key]}</span></Button>)}</div>;
}
...
export function Radar({ data }: { data: RadarData }) {
  ...
  const toggleSaved = (ad: Ad, section: Section) => { const key = adKey(ad); const next = savedMap.has(key) ? savedAds.filter((item) => adKey(item.ad) !== key) : [...savedAds, { ad, sections: [section] }]; setSavedAds(next); window.localStorage.setItem(SAVED_KEY, JSON.stringify(next)); };
  ...
  const singerShown = byStatus(status.singer === "saved" ? singerAll : data.singerSeeking, status.singer).sort((a, b) => Date.parse(b.date) - Date.parse(a.date));
  const interestingBase = byStatus(status.interesting === "saved" ? interestingAll : data.interesting, status.interesting).filter((ad) => [...filters].every((f) => matchesFilter(ad, f)));
  ...
  return <main className="min-h-screen bg-[#070a10] text-slate-100"><div className="radar-glow pointer-events-none fixed inset-0" /><div className="relative mx-auto max-w-6xl px-4 pb-20 pt-8 sm:px-6 sm:pt-12">
    ...
    <Tabs defaultValue="singer" className="gap-6"><TabsList className="h-auto w-full flex-wrap justify-start gap-1 overflow-visible rounded-xl border border-white/10 bg-white/[0.04] p-1.5 sm:w-fit"><TabsTrigger value="singer" className="min-h-10 gap-2 rounded-lg px-4 text-sm data-[state=active]:bg-cyan-300 data-[state=active]:text-slate-950">Zpěv hledá <span className="rounded-full bg-black/10 px-2 py-0.5 text-xs">{data.singerSeeking.length}</span>{newSinger > 0 && <span className="text-xs font-bold">+{newSinger} nové</span>}</TabsTrigger><TabsTrigger value="interesting" className="min-h-10 gap-2 rounded-lg px-4 text-sm data-[state=active]:bg-cyan-300 data-[state=active]:text-slate-950">Obecně zajímavé <span className="rounded-full bg-black/10 px-2 py-0.5 text-xs">{data.interesting.length}</span>{newInteresting > 0 && <span className="text-xs font-bold">+{newInteresting} nové</span>}</TabsTrigger></TabsList>
      <TabsContent value="singer" className="space-y-4"><div className="mb-5"><h2 className="text-2xl font-semibold tracking-tight text-white">Zpěv hledá kapelu nebo projekt</h2><p className="mt-1 text-slate-400">Pouze Praha a Středočeský kraj; bez coverů, tribute a zábavových kapel.</p></div><div className="mb-6 rounded-2xl border border-white/10 bg-white/[0.025] p-4"><StatusFilters value={status.singer} onChange={(value) => setStatus((s) => ({ ...s, singer: value }))} counts={counts(singerAll)} /></div>{cards(singerShown, "singer")}</TabsContent>
      <TabsContent value="interesting" className="space-y-4"><div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-2xl font-semibold tracking-tight text-white">Obecně zajímavé inzeráty</h2><p className="mt-1 text-slate-400">Moderní rock a metal, vlastní tvorba a nové projekty vhodné ke sledování, propojení nebo podpoře.</p></div><div className="shrink-0"><label className="mb-1.5 block text-sm font-semibold text-slate-400">Řazení</label><Select value={sort} onValueChange={setSort}><SelectTrigger className="min-w-44 border-white/10 bg-white/[0.035] text-slate-200"><SelectValue /></SelectTrigger><SelectContent className="border-white/10 bg-[#111821] text-slate-100"><SelectItem value="newest">Nejnovější</SelectItem><SelectItem value="score">Nejvyšší shoda</SelectItem></SelectContent></Select></div></div><div className="mb-6 space-y-3 rounded-2xl border border-white/10 bg-white/[0.025] p-4"><StatusFilters value={status.interesting} onChange={(value) => setStatus((s) => ({ ...s, interesting: value }))} counts={counts(interestingAll)} /><TopicFilters ads={status.interesting === "saved" ? interestingAll : data.interesting} active={filters} onToggle={toggleFilter} onClear={() => setFilters(new Set())} /></div>{cards(interestingShown, "interesting")}</TabsContent>
    </Tabs></div></main>;
}
